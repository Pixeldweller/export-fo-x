 ...
 * License 2.0) and changed: draws a shape's frame only where the shape
 * declares one, instead of always.
 *
...
//			} else {
				...
				if ( mso_position_vertical_relative !=null
					...
					applyShapeStroke(ret, shape); // a frame only where the shape declares one
					...
					if (margin_top==null) {
						...
						float marginTop = parsePtsVal(margin_top);
							...
							if (marginTop<=0) {
								...
								if (marginTopPositiveCase(foRenderer, props, doc, ret, writableWidthPts, modelContent.getChildNodes(), shape)) {
...
//			mso-position-horizontal-relative:text
	...
	private boolean marginTopPositiveCase(FORenderer foRenderer, Map<String, String> props, Document doc, Element ret, float widthPts,
			NodeList childNodes, org.docx4j.vml.VmlShapeElements shape) {
...
//			mso-position-horizontal-relative:text
		...
		if (mso_position_horizontal_relative==null) {
			...
			if ( mso_position_horizontal.equals("left")) {
				...
				applyShapeStroke(block, shape); // a frame only where the shape declares one
