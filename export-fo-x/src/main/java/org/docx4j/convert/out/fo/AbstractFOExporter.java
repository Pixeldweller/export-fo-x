 ...
 * License 2.0) and changed: the finished XSL-FO is passed through
 * FoAbsolutePositionHoister before it reaches the renderer.
 *
...
public abstract class AbstractFOExporter extends AbstractWmlExporter<FOSettings, FOConversionContext>{
	...
	//				   			String.format("%04x", (int) c2));					
		...
		// export-fo-x: repair FO that FOP would otherwise mislay. In particular, FOP
		// silently discards an absolutely positioned fo:block-container nested inside an
		// fo:inline -- which is exactly where the docx-to-FO transform puts a shape that
		// came from a w:pict inside a w:r. Done here because this is the one point where
		// the whole FO document exists in one piece, before the renderer and before the
		// dump file, so what gets debugged is what gets rendered.
		foDocument = org.docx4j.convert.out.fo.x.FoAbsolutePositionHoister.process(foDocument);
