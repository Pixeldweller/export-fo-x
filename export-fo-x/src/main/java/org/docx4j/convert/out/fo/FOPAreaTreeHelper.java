 ...
 * License 2.0) and changed: header/footer extent calculation no longer throws
 * NumberFormatException on blocks without @bpda; several integer divisions and
 * unchecked casts corrected.
 *
...
//        foSettings.getFeatures().add(ConversionFeatures.PP_PDF_APACHEFOP_DISABLE_PAGEBREAK_LIST_ITEM); // in 3.0.1, this is off by default
    		...
    		if (pageSequence instanceof Element ) {
    	    	...
    	    	for (int j = 0 ; j <pageSequence.getChildNodes().getLength(); j++ ) {
    	    		...
    	    		if (pageViewport instanceof Element ) {
	        			...
	        			Element page = firstChildElement(pageViewport);
	        			if (page == null) {
	        				log.debug("No page area for {}", simplePageMasterName);
	        				continue;
	        			}
	        	    	...
	        	    	for (int k = 0 ; k <page.getChildNodes().getLength(); k++ ) {
	        	    		...
	        	    		if (regionViewport instanceof Element ) {
	    	        			...
	    	        			Element region = firstChildElement(regionViewport);
	    	        			if (region == null) {
	    	        				continue;
	    	        			}
    ...
    /**
     * Height this block contributes to its region.
     *
     * <p>An absolutely positioned block-container -- how export-fo-x renders a
     * page-anchored letterhead -- is out of flow, and FOP reports it with no @bpda at
     * all. docx4j's unguarded {@code Integer.parseInt("")} therefore threw a
     * NumberFormatException for every such shape on every page, and the header ended up
     * sized from whatever survived. Out-of-flow content must not count towards a region's
     * extent anyway, so zero is both safe and correct here.
     */
    private static int blockExtent(Element block, String simplePageMasterName) {

    	String bpda = block.getAttribute("bpda");
    	if (bpda == null || bpda.isEmpty()) {
    		if (log.isDebugEnabled()) {
    			log.debug("{}: block with no @bpda (out of flow), contributing nothing to the extent: {}",
    					simplePageMasterName, XmlUtils.w3CDomNodeToString(block));
    		}
    		return 0;
    	}
    	try {
    		return Integer.parseInt(bpda.trim());
    	} catch (java.lang.NumberFormatException nfe) {
    		log.warn("{}: unreadable @bpda '{}', treating it as zero", simplePageMasterName, bpda);
    		return 0;
    	}
    }

    /** First child that is an element, skipping whitespace text nodes. */
