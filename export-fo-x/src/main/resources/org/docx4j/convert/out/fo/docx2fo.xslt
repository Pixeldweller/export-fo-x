...
<!--
  ...
  Taken from docx4j-export-fo 11.5.9 (Copyright Plutext Pty Ltd, Apache License
  2.0) and changed: the w:pict template now hands every picture to FOPictWriter
  instead of only the three shapes it recognised, so v:line and untexted v:rect
...
<xsl:template match="w:pict">
	...
	<xsl:choose>
		...
		<xsl:otherwise>
			<!--
			  export-fo-x: every other w:pict is handed to FOPictWriter rather than dropped.
			  docx4j only recognised ./v:shape/v:textbox and ./v:rect/v:textbox here and
			  reported "NOT IMPLEMENTED: ... without v:imagedata" for the rest, which is what
			  loses a letterhead's v:line fold marks and its untexted v:rect decorations.
			  The Java side decides what each shape is; the textbox content (if any) is
			  converted here so that it arrives already in FO.
			-->
			<xsl:variable name="childResults">
